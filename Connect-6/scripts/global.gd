...
class Board:
	...
	var u_eval = []			# 右上方向評価値
	var d_eval = []			# 右下方向評価値
	#var h_eval_stack = []			# 水平方向評価値スタック
	#var v_eval_stack = []			# 垂直方向評価値スタック
	#var u_eval_stack = []			# 右上方向評価値
	#var d_eval_stack = []			# 右下方向評価値
	var move_hist = []				# 着手履歴
	...
	func _init():
		...
		#h_eval_stack.resize(N_VERT)
		#v_eval_stack.resize(N_HORZ)
		#u_eval_stack.resize(N_DIAGONAL)
		#d_eval_stack.resize(N_DIAGONAL)
		clear()
		#
		unit_test()
	func clear():
	...
	func eval_putxy(x, y, next_color):
		eval -= h_eval[y]
		#h_eval_stack[y].push_back(h_eval[y])
		...
		#
		eval -= v_eval[x]
		#v_eval_stack[y].push_back(v_eval[y])
		...
		if ix >= 0:
			eval -= u_eval[ix]
			#u_eval_stack[y].push_back(u_eval[y])
		...
		if ix >= 0:
			eval -= d_eval[ix]
			#d_eval_stack[y].push_back(d_eval[y])
